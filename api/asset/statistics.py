...
@router.post("/vul")
async def vulnerability_data_statistics_vulname(request_data: dict, db=Depends(get_mongo_db), _: dict = Depends(verify_token)):
    query = await get_search_query("vulnerability", request_data)
    if query == "":
        return {"message": "Search condition parsing error", "code": 500}
    pipeline = [
        {
        ...
        },
        {
            "$facet": {
                "by_vulname": [
                    {"$group": {"_id": "$vulname", "num_tutorial": {"$sum": 1}}},
                    {"$match": {"_id": {"$ne": None}}}
                ]
            }
        }
    ...
    for r in result:
        for vulname in r['by_vulname']:
            vulname_list[vulname["_id"]] = vulname["num_tutorial"]

    vulname_list = dict(sorted(vulname_list.items(), key=lambda item: -item[1]))
    for vulname in vulname_list:
        result_list['Vul'].append({"value": vulname, "number": vulname_list[vulname]})
    return {
        "code": 200,
        "data": result_list
    }
