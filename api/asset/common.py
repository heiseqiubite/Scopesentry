...
async def delete_all_data(request_data: dict, db=Depends(get_mongo_db), _: dict = Depends(verify_token)):
    try:
        ...
        # 构建查询条件，使用与其他接口相同的搜索逻辑
        request_data_for_query = {
            "search": search,
            "filter": filter_data
        }
        query = await get_search_query(index, request_data_for_query)
        if query == "":
            return {"message": "Search condition parsing error", "code": 500}
        
        # 如果没有查询条件，为了安全起见，不允许删除所有数据
        if len(query) == 0:
            return {"code": 400, "message": "Cannot delete all data without search conditions"}
        
        # 执行批量删除操作
        result = await db[index].delete_many(query)
        
        if result.deleted_count > 0:
            return {"code": 200, "message": f"Successfully deleted {result.deleted_count} records", "deletedCount": result.deleted_count}
        else:
            return {"code": 404, "message": "No data found matching the criteria"}
