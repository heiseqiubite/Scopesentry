...
async def poc_data(request_data: dict, db=Depends(get_mongo_db), _: dict = Depends(verify_token)):
    try:
        ...
        cursor: AsyncIOMotorCursor = db.PocList.find(query, {"_id": 0, "id": {"$toString": "$_id"}, "name": 1, "level": 1, "time": 1, "tags": 1}).sort([("time", DESCENDING), ("_id", DESCENDING)]).skip((page_index - 1) * page_size).limit(page_size)
        # 获取结果并处理缺失的 tags 字段
